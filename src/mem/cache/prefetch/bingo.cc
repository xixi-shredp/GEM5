                         ...
                         const CacheAccessor &cache)
{
    ...
    if (ftFind(region) == nullptr) {
        ...
        for (unsigned i = 0; i < patternLen; i++) {
            ...
            addresses.push_back(AddrPriority(pfAddr, prio--));
...
{
    Addr blockNo = info.addr >> lBlkSize;
    Addr region = blockNo / patternLen;

    // stats: overpredictions for tracked prefetched blocks that never got a
    // demand access before being evicted.
    auto pfIt = prefetchedBlocks.find(blockNo);
    if (pfIt != prefetchedBlocks.end()) {
        if (pfIt->second == PC_ADDRESS) {
            bingoStats.overpredFromPcAddress++;
        } else if (pfIt->second == PC_OFFSET) {
            bingoStats.overpredFromPcOffset++;
        }
        prefetchedBlocks.erase(pfIt);
    }

    // Filter table: just erase (region only saw one offset -> no PHT train)
    ftErase(region);

    // Accumulation table: train PHT then erase
    ATEntry entry = atErase(region);
    if (entry.valid) {
        Addr vBlock = entry.region * patternLen + entry.offset;
        phtInsert(entry.pc, vBlock, entry.pattern);
        bingoStats.llcEvictTrain++;
    }

    // If no other offsets of this region still live, drop its event tag.
    if (ftMap.count(region) == 0 && atMap.count(region) == 0) {
        regionEvent.erase(region);
    }
}
