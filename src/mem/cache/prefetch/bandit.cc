...
void
Bandit::setParentInfo(System *sys, ProbeManager *pm, unsigned blk_size)
{
    Multi::setParentInfo(sys, pm, blk_size);
    // Multi skips Base::setParentInfo, so Bandit registers itself here to
    // receive demand-access probes (used to drive the bandit step counter).
...
void
Bandit::notify(const CacheAccessProbeArg &arg, const PrefetchInfo &pfi)
{
    stepAccesses++;
