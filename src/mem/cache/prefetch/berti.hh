...
class BertiPrefetcher : public Queued
{
  ...
  protected:
    struct HistoryInfo
    {
        ...
        bool
        operator==(const HistoryInfo &rhs) const
        {
    ...
    class TableOfDeltasEntry
    {
      public:
        ...
        void
        resetConfidence(bool reset_status)
        {
            counter = 0;
        ...
        updateStatus()
        {
            ...
            for (auto &info : deltas) {
                ...
                info.status =
                    (info.coverageCounter >= 4) ? L1_PREF : info.status;
        ...
        TableOfDeltasEntry(int size) { deltas.resize(size); }
    };
    ...
    class HistoryTableEntry : public TableOfDeltasEntry, public TaggedEntry
    {
      public:
        ...
        HistoryTableEntry(int deltaTableSize)
    ...
    void searchTimelyDeltas(HistoryTableEntry &entry, const Cycles &latency,
                            const Cycles &demand_cycle, const Addr &blk_addr);

    void
    printDeltaTableEntry(const TableOfDeltasEntry &entry)
    {
        DPRINTF(BertiPrefetcher, "Entry Counter: %d\n", entry.counter);
        for (auto &info : entry.deltas) {
            DPRINTF(BertiPrefetcher, "=>[delta: %d coverage: %d status: %d]\n",
  ...
  public:
    ...
    getEvictBestDelta()
    {
        return evictedBestDelta;
    }

    int
    getBestDelta()
    {
        return lastUsedBestDelta;
    }
