...
class Kairos : public Queued
{
  private:
    ...
    partitioning_policy::WayPartitioningPolicy *llcPartition;
    ...
    // Cached reference to the surrounding cache's accessor, used to send
    // metadata traffic out the mem-side when enableLlcMetadata is on.
    CacheAccessor *lastCache = nullptr;
    RequestorID metadataRequestorId = 0;
    // Current set of LLC ways assigned to metadata (for PID-driven resize).
    std::list<unsigned> llcMetadataWays;
