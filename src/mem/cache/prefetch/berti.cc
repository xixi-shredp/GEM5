...
BertiPrefetcher::BertiStats::BertiStats(statistics::Group *parent)
    : statistics::Group(parent),
      ADD_STAT(trainOnHit, statistics::units::Count::get(),
               "Count of train on hit"),
      ADD_STAT(trainOnMiss, statistics::units::Count::get(),
               "Count of train on miss"),
      ADD_STAT(notifySkippedCond1, statistics::units::Count::get(),
               "Count of notify skipped on mixed condition"),
      ADD_STAT(notifySkippedIsPF, statistics::units::Count::get(),
               "Count of notify skipped isPF"),
      ADD_STAT(notifySkippedNoEntry, statistics::units::Count::get(),
               "Count of notify skipped no Berti entry"),
      ADD_STAT(entryEvicts, statistics::units::Count::get(),
               "Count of Berti entry evicted")
{}
...
BertiPrefetcher::updateHistoryTable(const PrefetchInfo &pfi)
{
    ...
    HistoryInfo new_info = {.vAddr = training_addr, .timestamp = curCycle()};
    ...
    if (entry) {
        ...
        bool found_addr_in_hist =
            std::find(entry->history.begin(), entry->history.end(),
                      new_info) != entry->history.end();
        if (!found_addr_in_hist) {
            ...
            DPRINTF(BertiPrefetcher,
                    "PC=%lx, addr %lx found in history table hit, ignore\n",
                    pfi.getPC(), training_addr);
        ...
        if (entry->hysteresis) {
            ...
            historyTable.insertEntry(pcHash(entry->pc), entry->isSecure(),
                                     entry, false);
        } else {
            if (entry->bestDelta.status != NO_PREF) {
                int64_t blk_delta =
                    (int64_t)blockIndex(pfi.getAddr() +
                ...
                evictedDeltas[blk_delta] = evictedDeltas.count(blk_delta)
                                               ? evictedDeltas[blk_delta] + 1
                                               : 1;
            }
            ...
            entry->history.clear();
            entry->history.push_back(new_info);
            historyTable.insertEntry(pcHash(pfi.getPC()), pfi.isSecure(),
                                     entry);
        }
    }
    return nullptr;
...
void
BertiPrefetcher::searchTimelyDeltas(HistoryTableEntry &entry,
                                    const Cycles &search_latency,
                                    const Cycles &demand_cycle,
                                    const Addr &trigger_addr)
{
    DPRINTF(BertiPrefetcher,
            "latency: %lu, demand_cycle: %lu, history count: %lu\n",
            search_latency, demand_cycle, entry.history.size());
    ...
    for (auto it = entry.history.rbegin(); it != entry.history.rend(); it++) {
        ...
        if (it->timestamp + search_latency >= demand_cycle) {
            DPRINTF(BertiPrefetcher,
                    "skip untimely delta: %lu + %lu <= %u : %ld\n",
                    it->timestamp, search_latency, demand_cycle, delta);
            continue;
        }
        assert(delta != 0);
        new_deltas.push_back(delta);
        DPRINTF(BertiPrefetcher, "Timely delta found: %d=(%x - %x)\n", delta,
                trigger_addr, it->vAddr);
    ...
    for (auto &delta : new_deltas) {
        ...
        for (auto &delta_info : entry.deltas) {
            if (delta_info.coverageCounter != 0 && delta_info.delta == delta) {
                ...
                DPRINTF(BertiPrefetcher,
                        "Inc coverage for delta %d, cov = %d\n", delta,
                        delta_info.coverageCounter);
                miss = false;
                break;
            }
            ...
            if (entry->bestDelta.status != NO_PREF) {
                ...
                Addr pf_addr =
                    useByteAddr
                        ...
                        : (blockIndex(pfi.getAddr()) + entry->bestDelta.delta)
                              << lBlkSize;
                sendPFWithFilter(pfi, pf_addr, addresses, 32,
                                 entry->bestDelta.coverageCounter >= 8);
                // Do not know the usage below:
                                  ...
                                  bool using_best_delta_and_confident)
{
    ...
    if (trainBlockFilter.contains(addr)) {
        ...
        int64_t blk_delta =
            (int64_t)blockIndex(addr) - blockIndex(pfi.getAddr());
        topDeltas[blk_delta] =
...
void
BertiPrefetcher::notifyFill(const CacheAccessProbeArg &arg)
{
    auto pkt = arg.pkt;
    if (pkt->req->isInstFetch() || !pkt->req->hasVaddr() ||
        !pkt->req->hasPC()) {
    ...
    DPRINTF(BertiPrefetcher, "Cache Fill: %s isPF: %d, pc: %lx\n",
    ...
    HistoryTableEntry *entry = historyTable.findEntry(
        pcHash(pkt->req->getPC()), pkt->req->isSecure());
    if (!entry) {
