      ...
      kairosStats(this)
{
    ...
    if (enableLlcMetadata) {
        // Register a distinct requestor name so the LLC PartitionManager
        // can recognize metadata traffic.
        metadataRequestorId = p.sys->getRequestorId(this, "metadata");
    }
    // Initialize the active metadata way list from params (for PID resize).
    for (unsigned w = 0; w < p.llc_metadata_initial_ways.size(); ++w) {
        llcMetadataWays.push_back(p.llc_metadata_initial_ways[w]);
    }
}
...
{
    if (!enableLlcMetadata || lastCache == nullptr) {
        return;
    }
    const Addr paddr = metadataAddrFor(last_addr);
    lastCache->sendMetadataRequest(paddr, is_write, metadataRequestorId);
    if (is_write) {
...
Kairos::resizeMetadata(int8_t direction)
{
    ...
    // Drive the real LLC partition: one physical LLC way per WAY_CHUNK
    // shadow ways (mirrors the original croage::resize_metadata quantum).
    if (llcPartition && !llcMetadataWays.empty()) {
        const unsigned target_phys_ways = std::max<unsigned>(
            llcMetadataWaysMin,
            std::min<unsigned>(llcMetadataWaysMax, currentWays / WAY_CHUNK));
        while (llcMetadataWays.size() > target_phys_ways) {
            // Shrink: return the most recently added way to data partition.
            unsigned w = llcMetadataWays.back();
            llcMetadataWays.pop_back();
            llcPartition->removeWayToPartition(/*metadata=*/1, w);
            llcPartition->addWayToPartition(/*data=*/0, w);
        }
        while (llcMetadataWays.size() < target_phys_ways) {
            // Grow: steal the highest-numbered data way.
            unsigned candidate = 0;
            bool found = false;
            // We don't have a direct API to query data ways, so walk
            // candidate way indices from high to low until we find one
            // not already in llcMetadataWays.
            for (unsigned w = llcMetadataWaysMax + llcMetadataWaysMin;
                 w-- > 0;) {
                bool used = false;
                for (unsigned m : llcMetadataWays) {
                    if (m == w) {
                        used = true;
                        break;
                    }
            ...
            llcMetadataWays.push_back(candidate);
            llcPartition->removeWayToPartition(/*data=*/0, candidate);
            llcPartition->addWayToPartition(/*metadata=*/1, candidate);
        }
    }
}
                          ...
                          const CacheAccessor &cache)
{
    ...
    // Remember the accessor so the internal helpers can emit LLC traffic.
    lastCache = const_cast<CacheAccessor *>(&cache);

    Addr line_addr = blockIndex(pfi.getAddr());
    Addr pc = pfi.getPC();
    bool cache_miss = pfi.isCacheMiss();
    bool useful_prefetch =
        !cache_miss && cache.hasBeenPrefetched(pfi.getAddr(), pfi.isSecure());
    ...
    for (Addr line : candidates) {
        ...
        addresses.push_back(AddrPriority(pf_addr, prio++));
