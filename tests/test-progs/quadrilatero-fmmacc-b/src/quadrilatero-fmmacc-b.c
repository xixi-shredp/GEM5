...
#define ENCODE_FMMACC_B(md, ms1, ms2) \
...
static inline void
matrix_fmmacc_b_m2_m1_m0(void)
{
    asm volatile(".word %0" : : "i"(ENCODE_FMMACC_B(2, 1, 0)) : "memory");
}
...
get_byte_lane(uint32_t value, int lane)
{
    return (int8_t)(value >> (lane * 8));
}
        ...
        for (int col = 0; col < 4; ++col) {
            ...
            for (int word = 0; word < 4; ++word) {
                for (int lane = 0; lane < 4; ++lane) {
                    acc += (float)get_byte_lane(a_bits[row][word], lane) *
                           (float)get_byte_lane(b_bits[col][word], lane);
                }
            }
            ref[row][col] = acc;
        }
